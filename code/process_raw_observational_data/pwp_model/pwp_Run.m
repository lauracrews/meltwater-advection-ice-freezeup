function[model]=pwp_Run(config,init,forc)
%pwp_Run PWP Model
...
if Kz*dt./(dz)^2>0.5
	...
	'Decrease Kz to ' num2str(.5*(dz)^2/dt,'%0.2e') ...
  ', decrease dt to ' num2str(.5*(dz)^2/Kz,'%0.0f') ...
	', or increase dz to ' num2str(sqrt(2*Kz*dt),'%0.1f') '.'])
...
clear T S UV Told Sold UVold mld i delta_sig D vd G Ri rmin u v Sigm Tm Sm UVm timenow j jj
clear ang *_zz Sig nn
...
clear fid*
clear lat Kz Km r tintv zmax tstart tstop I0 lambda rho BRiCrit GRiCrit plotflag writeout
