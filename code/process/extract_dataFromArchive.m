function [metData, wvdata, profiles] = extract_DataFromArchive()

    if ~exist([rootPath, 'data/', 'healyUnderway.csv'], 'file') || ...
            ~exist([rootPath, 'data/', 'waveglider.nc'], 'file') || ...
            ~exist([rootPath, 'data/', 'profiles.nc'], 'file')
    ...
    %% Read Healy Underway data into a struct
    metData = table2struct(readtable([rootPath, 'data/', 'healyUnderway.csv']), 'toScalar', true);

    %% Read Wave Glider data into a struct
    curFile = [rootPath, 'data/', 'waveglider.nc'];
    ...
    %% Read Seaglider and uCTD data into a struct
    curFile = [rootPath, 'data/', 'profiles.nc'];
    ...
    inds = find(~strcmp('uCTD', dataset) == 1);
    for i = 1:length(inds)
